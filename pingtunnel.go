...
func recvICMP(workResultLock *sync.WaitGroup, exit *bool, conn icmp.PacketConn, recv chan<- *Packet, cryptoConfig *CryptoConfig, keyProvider func() []int) {
	...
	for !*exit {
		...
		var myMsg *MyMsg
		// Helper to try decrypting with a config
		tryDecrypt := func(config *CryptoConfig) *MyMsg {
			data := payloadData
			if config != nil {
				var err error
				data, err = config.Decrypt(data)
				if err != nil {
					return nil
				}
			}
			...
			if m.Magic != (int32)(MyMsg_MAGIC) {
				return nil
			}
			return m
		}

		// 1. Try default config (global key or plaintext)
		myMsg = tryDecrypt(cryptoConfig)

		// 2. Future extension: per-user encryption keys via keyProvider
		_ = keyProvider

		if myMsg == nil {
			// loggo.Debug("processPacket data invalid or decrypt failed")
			continue
		}

		recv <- &Packet{my: myMsg,
